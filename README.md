...
- ✅ **图像校正**：背景矫正与暗场/亮场(Dark/Flat)校正，消除背景噪声、探测器暗电流和光强不均
- ✅ **几何参数配置**：灵活设置源物距(SOD)、源探距(SDD)、投影数量、探测器像素尺寸等
- ✅ **参数校正**：支持旋转中心偏移校正、探测器面内偏转校正、探测器偏移校正等
- ✅ **校正可视化**：界面展示当前背景/平场校正公式，并在投影预览中显示旋转中心线
...
工作区已有 `proj` 文件夹，里面包含 360 张投影图，以及 `dark_0000.tif`、`flat_0000.tif`。软件初始参数保持为空/0，导入数据后会自动读取投影数量；这组示例数据可参考视频中的仿真参数设置：
...
2. **选择投影目录**
   - 启动时投影目录为空，软件不会自动加载任何数据
   - 点击"选择目录"按钮选择包含投影图像的文件夹
   - 或直接在文本框输入路径
   - 点击"导入并显示"查看投影图像

3. **配置重建参数**
   - **重建参数标签**：配置源物距、源探距、探测器像素尺寸等基本几何参数
   - 投影数量由导入目录中的投影图像数量自动决定
   - 旋转中心(pixel)显示在 CT 重建页，可直接输入偏移值，也可点击“自动计算旋转中心”按投影主体轮廓获取初值
   - **体数据设置标签**：配置输出体数据的大小和分辨率
   - 点击“计算体素尺寸”可按 `探测器像素尺寸 × SOD / SDD` 自动计算物体平面的体素尺寸，并按投影尺寸匹配体数据范围
   - **参数校正标签**：配置背景矫正、dark/flat、探测器面内偏转和探测器偏移等校正参数
...
- **投影数量**：扫描获取的投影图像总数，由导入目录中的投影图像数量自动决定
...
- **旋转中心偏移**：旋转中心在投影图像中的像素位移。用于补偿机械对齐误差
  - 投影预览会以红色竖线显示当前旋转中心位置，便于观察偏移效果
  - 自动计算会先应用可用的背景/dark/flat校正，再根据多角度投影主体轮廓估计校正量
- **背景矫正**：启用时自动加载 `background_*`、`bg_*` 或 `bkg_*` 背景图像并从投影中扣除
  - 背景矫正不单独设置保存路径，结果随重建输出统一保存到“保存目录”
...
- **体素大小**：单个体素的物理尺寸，单位mm
  - 软件初始值为 `0`；输入 SOD、SDD 和探测器像素尺寸后，可由 `0.2 × 200 / 800` 得到示例值 `0.05 mm`
  - 示例数据推荐体数据尺寸为 `512 x 512 x 512`，对应物体平面视野 `25.6 mm`
  - 若体素设为 `0.05 mm` 但体数据仍为 `256 x 256 x 256`，视野只有 `12.8 mm`，会和视频中的 512 输出不一致，并造成裁剪或放大后的显示差异
...
1. **背景矫正**（可选）
   - 若目录内存在 `background_*`、`bg_*` 或 `bkg_*` 图像，会先从每张投影中扣除背景
   - 扣除后的负值会截断为0，避免进入后续对数变换

2. **暗场/亮场校正**（可选）
   - 界面展示的完整处理公式为：`μ = -ln((max(I - B, 0) - D) / (F - D))`
